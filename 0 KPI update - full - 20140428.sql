...
#20140418 added theme and epic mapping
#20140418 created views
...
create or replace view view_epicthememapping as (select * from  ENO_themeEpicMapping);
...
create or replace view view_kpi_all as (select * from ENO_kpi_all);
...
#20140424 changed owner (Joost -> Jeroen)
UPDATE ENO_worklists wl
	SET wl.workitem = 'No Sprint filled, status Specification',
		wl.priority = 'High',
        wl.actionholder = 'Jeroen ter Horst'
	WHERE
...
create or replace view view_worklists_selection as (select * from ENO_worklists where workitem is not null);
create or replace view view_worklists_all as (select * from ENO_worklists);
...
create or replace view view_voorraad as (select * from ENO_voorraad order by id);
...
create or replace view view_sareadychanges as (select 
	issueid, 
	author, 
...
# master data table
drop table if exists ENO_saready;
create table if not exists ENO_saready as (
select 
	ji.id, 
	ji.pkey, 
	ji.summary, 
...
create or replace view view_saready as (select * from ENO_saready);
...
update ENO_statustimes st
	left join (
		...
		group by sct.id) as sctt on sctt.id = st.stid
...
;describe ENO_themeEpicMapping;

create or replace view view_dump_all as (
select st.*, vr.Sum50TeamEffort, vr.Sum70TeamEffort, ki.*, tEM.ThemeKey, tEM.ThemeSummary, tEM.ThemeStatus, tEM.ThemeSize from ENO_statustimes st 
	left join ENO_voorraad vr on vr.id = st.stid
	left join ENO_kpi_all ki on st.stid = ki.id
	left join ENO_themeEpicMapping tEM on st.stid = tEM.EpicId
	group by st.stid
);

create or replace view view_statustimes as (
select st.* from ENO_statustimes st);
...
create or replace view view_b2ball as select * from ENO_B2Bmaster;
...
SELECT * from view_dump_all INTO OUTFILE '/tmp/9dumpall.jiradump';
#select * from view_epicthememapping limit 10000;

SELECT * from view_dump_all limit 100000;
